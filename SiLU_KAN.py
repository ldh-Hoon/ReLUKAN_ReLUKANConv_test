...
class SiLUKANLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.silu(x - self.phase_low)
        x2 = self.silu(self.phase_height - x)
        x = x1 * x2 * self.r
        x = x * x
...
class SiLUKAN(nn.Module):
    def __init__(self, width, grid, k):
        ...
        for i in range(len(width) - 1):
            self.rk_layers.append(SiLUKANLayer(width[i], grid, k, width[i+1]))
            # if len(width) - i > 2:
